...
abstract class ThinkOauth{
	...
	public function __construct($token = null){
		...
		$this->Type = strtoupper(substr($class, strripos($class, '\\')+1));
		//获取应用配置
		$config = \think\Config::get("THINK_SDK_{$this->Type}");
    ...
    public static function getInstance($type, $token = null) {
    	...
    	if (class_exists('Addons\\ThirdLogin\\Lib\\'.$name)) {
    		$class = 'Addons\\ThirdLogin\\Lib\\'.$name;
    		return new $class($token);
    	} else {
    		throw_exception(lang('_CLASS_NOT_EXIST_') . ':' . $name);
    	}
    }
