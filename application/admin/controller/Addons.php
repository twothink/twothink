 ...
 *
 * 消除了插件生成和管理的BUG： 由原来onethink的 tp3.2.3框架 的风格所致
 *修改： 洋洋洋, xdeepbreath@qq.com , 2017.1.3
 */
...
//		dump($list);
	...
	public function adminList($name) {
		...
		if (isset ( $uri_param [$key] )) {
			$map [$key] = array (	'like',	'%' . $uri_get [$key] . '%');
			unset ( $uri_param [$key] );
		}
		...
		if (isset ( $model )) {
			...
			$my_model = Modelmodel::get(['name'=>$model_name]);//获取插件对应的模型对象
			$this->assign ( 'model', $my_model);
