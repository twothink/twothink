...
class ModelApi {
    ...
    public static function get_model_by_id($id){
        return $model = \think\Db::name('Model')->getFieldById($id,'title');
    }
    ...
    public static function get_model_attribute($model_id, $group = true){
        ...
        if(!isset($list[$model_id])){
            ...
            $info = \think\Db::name('Attribute')->where($map)->select();
            $list[$model_id] = $info;
        ...
        if($group){
            $sort  = \think\Db::name('Model')->getFieldById($model_id,'field_sort');
